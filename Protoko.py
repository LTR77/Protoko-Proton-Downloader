...
def Authenticate_Github():
    global PATLabel, PATComboBox, PATButton, AuthDestroySecondCLick
    if AuthDestroySecondCLick == False:
        PATButton.place(x=390, y=225)
        PATLabel.place(x=370, y=165)
        PATComboBox.place(x=390, y=190)
        AuthDestroySecondCLick = True
    else:
        PATLabel.place_forget()
        PATComboBox.place_forget()
        PATButton.place_forget()
        AuthDestroySecondCLick = False

def Auth_GitHub_Check():
    global PATLabel, PATComboBox, PATButton, AuthDestroySecondCLick
    if AuthDestroySecondCLick:
        PATSelection = PATComboBox.get()
        os.environ["GITHUB_TOKEN"] = PATSelection
        print("log: Saving")
        token_file = open("github_token.txt", "w")
        token_file.write(PATSelection)
        token_file.close()
        print("log: Success")
        try:
            if os.path.exists("github_token.txt"):
...
def options():
    global destroy_text_on_second_click, entry_box, lbltelldelete, lblshowdirs, delallbtn_command, deleteallbtn, openGithubbtn, changeDownloadLocation, download_dir_selection, showDownloadLocation, authGithubbtn, cbshowdirs, delete_runner_btn, list_deletable_files, PATLabel, PATComboBox, PATButton, AuthDestroySecondCLick
    pathOfFiles = default_dir
    if destroy_text_on_second_click == False:
        PATLabel = ttk.Label(root, text="Enter your GitHub Personal Access Token")
        PATLabel.place(x=370, y=165)
        PATLabel.place_forget()
        PATVar = tkinter.StringVar()
        PATComboBox = ttk.Combobox(root, textvariable=PATVar)
        PATComboBox.place(x=390, y=190)
        PATComboBox.place_forget()
        PATButton = ttk.Button(root, text="Confirm Token", command=Auth_GitHub_Check)
        PATButton.place(x=390, y=225)
        PATButton.place_forget()
        ...
        changeDownloadLocation.place(x=20, y=210)
        showDownloadLocation = ttk.Label(root, text=download_dir_selection)
        showDownloadLocation.place(x=20, y=250)
        openGithubbtn = ttk.Button(root, text="Github 💻", command=openGH)
        openGithubbtn.place(x=260, y=170)
        authGithubbtn = ttk.Button(root, text="Auth Github", command=Authenticate_Github)
        authGithubbtn.place(x=370, y=130)
        deleteallbtn = ttk.Button(root, text="Delete all ❌", command=encapsulate_the_dangerous_variable)
        deleteallbtn.place(x=260, y=130)
